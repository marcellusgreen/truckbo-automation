 ...
 */
function generateJWT(user: any, company: any): string {
  return jwt.sign(
    { 
      userId: user.id, 
 ...
 */
router.post('/v1/auth/initialize-demo', authRateLimit, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const response = ApiResponseBuilder.success(
      ...
      'Demo authentication data initialized',
 ...
 */
router.post('/v1/auth/login', authRateLimit, async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/v1/auth/register', authRateLimit, async (req: Request, res: Response) => {
  const client = await pool.connect();
  ...
  try {
    ...
    if (existingCompanyResult.rows.length > 0) {
      const response = ApiResponseBuilder.conflict(
        'Company', 'name or email', companyName,
        { requestId: req.context?.requestId, version: req.context?.apiVersion }
      );
