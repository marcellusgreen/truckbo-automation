...
export class VehicleReconciler {
   ...
   */
  addDocument(documentData: any, metadata: { 
    fileName: string; 
    source: string; 
    uploadDate?: string; 
  }): { success: boolean; vehicleVIN?: string; conflicts?: ConflictRecord[]; warnings?: string[] } {
    
    try {
      console.log(`📄 Adding document: ${metadata.fileName}`);
      
      // Extract VIN from document
      const extractedVIN = this.extractVINFromDocument(documentData);
      if (!extractedVIN) {
        return {
          success: false,
   ...
   */
  private extractVINFromDocument(documentData: any): string | null {
    // Check multiple possible locations for VIN
    const vinSources = [
      documentData.vin,
      documentData.vinNumber,
      documentData.vehicle_vin,
  ...
  private inferDocumentType(documentData: any): DocumentRecord['documentType'] {
    ...
    return typeMap[docType] || (docType as DocumentRecord['documentType']) || 'other';
  }
  ...
  private extractDate(documentData: any, dateType: 'effective' | 'expiration' | 'issue'): string | undefined {
    const datePaths: { [key: string]: string[] } = {
      effective: ['effectiveDate', 'effective_date', 'startDate', 'issue_date'],
      expiration: ['expirationDate', 'expiry', 'expiration_date', 'due_date'],
      issue: ['issueDate', 'issue_date', 'issuedDate']
    };
