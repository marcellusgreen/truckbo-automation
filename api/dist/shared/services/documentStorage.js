...
class DocumentStorage {
    constructor() {
        Object.defineProperty(this, "context", {
            ...
            configurable: true,
            writable: true,
            value: { layer: 'storage', component: 'DocumentStorage' }
        });
    }
    /**
    ...
    async handleVehicleData(data, organizationId, documentId) {
        ...
        try {
            ...
            logger_1.logger.error('Failed to handle vehicle data', this.context, error);
    ...
    async handleDriverData(data, organizationId, documentId) {
        ...
        try {
            ...
            logger_1.logger.error('Failed to handle driver data', this.context, error);
    ...
    async updateVehicleFromExtractedData(vehicleId, data) {
        ...
        if (data.truckNumber) {
            ...
            values.push(data.truckNumber);
        }
        if (data.dotNumber) {
            updates.push(`dot_number = $${paramIndex++}`);
            values.push(data.dotNumber);
        }
        if (data.documentType === 'registration') {
            ...
            if (data.registrationState) {
                updates.push(`registration_state = $${paramIndex++}`);
                values.push(data.registrationState);
            }
            if (data.registrationExpirationDate) {
                updates.push(`registration_expiry = $${paramIndex++}`);
                values.push(data.registrationExpirationDate);
            }
            if (data.registeredOwner) {
                updates.push(`registered_owner = $${paramIndex++}`);
                values.push(data.registeredOwner);
            }
        }
        if (data.documentType === 'insurance') {
            if (data.insuranceCarrier) {
                updates.push(`insurance_carrier = $${paramIndex++}`);
                values.push(data.insuranceCarrier);
            }
            ...
            if (data.insuranceExpirationDate) {
                ...
                values.push(data.insuranceExpirationDate);
            }
            if (data.coverageAmount) {
                updates.push(`coverage_amount = $${paramIndex++}`);
                values.push(data.coverageAmount);
            }
        }
        if (updates.length > 0) {
    ...
    async updateDriverFromExtractedData(driverId, data) {
        ...
        if (data.documentType === 'cdl') {
            if (data.cdlNumber) {
                updates.push(`cdl_number = $${paramIndex++}`);
                values.push(data.cdlNumber);
            }
            if (data.cdlState) {
                updates.push(`cdl_state = $${paramIndex++}`);
                values.push(data.cdlState);
            }
            if (data.cdlClass) {
                updates.push(`cdl_class = $${paramIndex++}`);
                values.push(data.cdlClass);
            }
            if (data.cdlIssueDate) {
                updates.push(`cdl_issue_date = $${paramIndex++}`);
                values.push(data.cdlIssueDate);
            }
            if (data.cdlExpirationDate) {
                updates.push(`cdl_expiration_date = $${paramIndex++}`);
                values.push(data.cdlExpirationDate);
            }
            if (data.cdlEndorsements) {
                updates.push(`cdl_endorsements = $${paramIndex++}`);
                values.push(data.cdlEndorsements);
            }
            if (data.cdlRestrictions) {
                updates.push(`cdl_restrictions = $${paramIndex++}`);
                values.push(data.cdlRestrictions);
            }
        }
        if (data.documentType === 'medical_certificate') {
            if (data.medicalCertNumber) {
                updates.push(`medical_cert_number = $${paramIndex++}`);
                values.push(data.medicalCertNumber);
            }
            if (data.medicalIssueDate) {
                updates.push(`medical_cert_issued_date = $${paramIndex++}`);
                values.push(data.medicalIssueDate);
            }
            if (data.medicalExpirationDate) {
                updates.push(`medical_cert_expiration_date = $${paramIndex++}`);
                values.push(data.medicalExpirationDate);
            }
            if (data.examinerName) {
                updates.push(`medical_examiner_name = $${paramIndex++}`);
                values.push(data.examinerName);
            }
            if (data.examinerNationalRegistry) {
                updates.push(`medical_examiner_registry = $${paramIndex++}`);
                values.push(data.examinerNationalRegistry);
            }
            if (data.medicalRestrictions) {
                updates.push(`medical_restrictions = $${paramIndex++}`);
                values.push(data.medicalRestrictions);
            }
        }
        if (updates.length > 0) {
     ...
     */
    detectVehicleConflicts(existing, extracted) {
        const conflicts = [];
    ...
    async checkVehicleDuplicates(data, organizationId) {
        ...
        try {
            ...
            logger_1.logger.warn('Error checking for vehicle duplicates', this.context, error);
     ...
     */
    detectDriverConflicts(existing, extracted) {
        const conflicts = [];
    ...
    async checkDriverDuplicates(data, organizationId) {
        ...
        try {
            ...
            logger_1.logger.warn('Error checking for driver duplicates', this.context, error);
     ...
     */
    async getDocumentWithEntities(documentId) {
        const selectQuery = `
      SELECT
        d.*,
      ...
      WHERE d.id = $1
    `;
        const res = await (0, db_1.query)(selectQuery, [documentId]);
        return res.rows[0] || null;
    }
}
exports.documentStorage = new DocumentStorage();
