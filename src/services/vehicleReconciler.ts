...
import { standardizeVehicleData } from '../utils/fieldStandardization';

export interface DocumentRecord {
  id: string;
  fileName: string;
  documentType: 'registration' | 'insurance' | 'cdl' | 'medical_certificate' | 'inspection' | 'permit' | 'other';
  extractedData: any;
  flexibleValidation?: any;
  vin: string;
  uploadDate: string;
  processedDate: string;
  confidence: number;
  status: 'active' | 'superseded' | 'invalid' | 'under_review';
  
  // Compliance-specific dates
...
export interface ConflictRecord {
  type: 'date_mismatch' | 'data_inconsistency' | 'duplicate_document' | 'vin_mismatch';
...
export interface ComplianceHistory {
  ...
  timeline: Array<{
    date: string;
    document: string;
    action: 'added' | 'updated' | 'expired' | 'superseded' | 'renewed';
    details: string;
  }>;
...
export interface VehicleRecord {
  ...
  // Document management
  documents: Map<string, DocumentRecord>; // documentId -> DocumentRecord
  documentsByType: Map<string, DocumentRecord[]>; // documentType -> DocumentRecord[]
  
  // Compliance tracking
  complianceHistory: Map<string, ComplianceHistory>; // documentType -> ComplianceHistory
  complianceStatus: {
    registration: ComplianceStatus;
    insurance: ComplianceStatus;
    inspection: ComplianceStatus;
    cdl: ComplianceStatus;
    medical: ComplianceStatus;
...
export interface ComplianceStatus {
  status: 'current' | 'expires_soon' | 'expired' | 'missing' | 'invalid' | 'under_review';
...
export interface ReconcilerStats {
  ...
  documentsPerVehicle: { avg: number; min: number; max: number };
  complianceBreakdown: {
    compliant: number;
    nonCompliant: number;
    expiresSoon: number;
  ...
  expirationAlert: {
    ...
    expiresThisMonth: number;
  };
}

export class VehicleReconciler {
  private vehicles: Map<string, VehicleRecord> = new Map();
  private vinAliases: Map<string, string> = new Map(); // Maps alternative VINs to primary VIN
  private documentIndex: Map<string, string> = new Map(); // Maps documentId to VIN
    ...
    try {
      const data = {
        vehicles: Object.fromEntries(
          Array.from(this.vehicles.entries()).map(([vin, vehicle]) => [
            vin,
            {
              ...vehicle,
              documents: Object.fromEntries(vehicle.documents),
              documentsByType: Object.fromEntries(
                Array.from(vehicle.documentsByType.entries()).map(([type, docs]) => [type, docs])
              ),
  ...
  private loadFromStorage(): void {
    try {
      ...
      // Restore vehicles
      if (data.vehicles) {
        Object.entries(data.vehicles).forEach(([vin, vehicleData]: [string, any]) => {
          const vehicle: VehicleRecord = {
            ...vehicleData,
            documents: new Map(Object.entries(vehicleData.documents || {})),
            documentsByType: new Map(
              Object.entries(vehicleData.documentsByType || {}).map(([type, docs]) => [type, docs as DocumentRecord[]])
            ),
  ...
  async addDocument(documentData: any, metadata: { 
    ...
    source: string; 
    uploadDate?: string; 
  }): Promise<{ success: boolean; vehicleVIN?: string; conflicts?: ConflictRecord[]; warnings?: string[] }> {
   ...
   */
  getVehicleSummary(vin: string): VehicleRecord | null {
   ...
   */
  getExpiringSoon(days: number = 30): Array<{
    vin: string;
    ...
    const expiringSoon: Array<any> = [];
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() + days);

    this.vehicles.forEach((vehicle, vin) => {
      const expiringDocs: Array<any> = [];
  ...
  getStats(): ReconcilerStats {
    ...
    this.vehicles.forEach(vehicle => {
      ...
      switch (vehicle.complianceStatus.overall) {
        case 'compliant': stats.complianceBreakdown.compliant++; break;
        case 'non_compliant': stats.complianceBreakdown.nonCompliant++; break;
        case 'expires_soon': stats.complianceBreakdown.expiresSoon++; break;
        case 'review_needed': stats.complianceBreakdown.needsReview++; break;
        case 'incomplete': stats.complianceBreakdown.incomplete++; break;
      }
   ...
   */
  private async extractVINFromDocument(documentData: any): Promise<string | null> {
    // Use standardization utilities to properly extract VIN
    // Standardize the document data to get VIN in consistent format
    const standardizedData = standardizeVehicleData(documentData, 'document_processing');
    ...
    if (standardizedData.vin && typeof standardizedData.vin === 'string' && standardizedData.vin.length >= 15) {
      ...
      if (cleanedVIN) return cleanedVIN;
    }
    ...
    const vinSources = [
      ...
      documentData.flexibleValidation?.extracted_fields?.find((f: any) => f.field.toLowerCase().includes('vin'))?.value,
   ...
   */
  private cleanAndValidateVIN(vin: string): string | null {
   ...
   */
  private resolvePrimaryVIN(vin: string): string {
   ...
   */
  private createNewVehicleRecord(vin: string): VehicleRecord {
   ...
   */
  private createDocumentRecord(documentData: any, metadata: any, vin: string): DocumentRecord {
    const now = new Date().toISOString();
   ...
   */
  private inferDocumentType(documentData: any): DocumentRecord['documentType'] {
    ...
    const docType = documentData.documentType || 
                   ...
                   documentData.flexibleValidation?.extracted_fields?.find((f: any) => f.field === 'document_type')?.value ||
                   'other';
    
    console.log('🎯 Extracted document type:', docType);
    
    // Map common variations to standard types
    const typeMap: { [key: string]: DocumentRecord['documentType'] } = {
    ...
    const finalType = typeMap[docType] || inferredFromFileName || (docType as DocumentRecord['documentType']) || 'other';
   ...
   */
  private extractDate(documentData: any, dateType: 'effective' | 'expiration' | 'issue'): string | undefined {
    const datePaths: { [key: string]: string[] } = {
    ...
    for (const path of paths) {
      ...
      const dateValue = documentData.extractedData?.[path] || 
                       documentData[path] ||
                       documentData.dates?.find((d: any) => d.type?.includes(dateType))?.date ||
                       documentData.flexibleValidation?.extracted_fields?.find((f: any) => 
                         f.field === path || f.field === `${dateType}_date` || f.field === `${dateType}Date`
                       )?.value;
   ...
   */
  private addDocumentToVehicle(vehicle: VehicleRecord, document: DocumentRecord): void {
   ...
   */
  private detectConflicts(vehicle: VehicleRecord, newDocument: DocumentRecord): ConflictRecord[] {
   ...
   */
  private updateComplianceStatus(vehicle: VehicleRecord, document: DocumentRecord): void {
   ...
   * Update overall compliance status
   */
  private updateOverallComplianceStatus(vehicle: VehicleRecord): void {
    const statuses = [
   ...
   * Update compliance history
   */
  private updateComplianceHistory(vehicle: VehicleRecord, document: DocumentRecord): void {
    const docType = document.documentType;
   ...
   */
  private calculateComplianceScore(vehicle: VehicleRecord): void {
    let totalScore = 0;
    let categories = 0;
    
    const complianceCategories = ['registration', 'insurance', 'inspection', 'cdl', 'medical'] as const;
    ...
    complianceCategories.forEach(category => {
      ...
      switch (status.status) {
        case 'current': categoryScore = 100; break;
        case 'expires_soon': categoryScore = 80; break;
        case 'under_review': categoryScore = 60; break;
        case 'invalid': categoryScore = 40; break;
        case 'expired': categoryScore = 0; break;
        case 'missing': categoryScore = status.confidence > 0 ? 20 : 0; break;
      }
   ...
   */
  private refreshVehicleComplianceStatus(vehicle: VehicleRecord): void {
    const now = new Date();
   ...
   */
  private generateWarnings(vehicle: VehicleRecord, document: DocumentRecord): string[] {
...
export const vehicleReconciler = new VehicleReconciler();